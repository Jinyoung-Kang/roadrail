...
**출발지 → 도착지**를 전국 어디든(지역 · 기차역 · 장소) 검색해 고르면, 자동차는 카카오 경로 예측으로,
...
>
> **데이터 원칙** — 화면의 값은 공식 API 가 준 값 · 그 값으로 계산한 통계 · 명시한 예측만 씁니다. 모르는 값을 규칙이나 짐작으로 채우지 않습니다(예: 열차 차종은 TAGO 시간표에 있을 때만, 돌발 위치는 안내에 좌표가 있을 때만). 값별 출처와 남겨 둔 추정 2가지의 이유: [docs/DATA-PROVENANCE.md](docs/DATA-PROVENANCE.md).
...
<tr>
<td width="50%"><img src="docs/images/road-route.png" alt="도로 분석"><br><sub><b>도로 분석 (전국)</b> · 강남역 → 전주시: 추천 경로 vs 고속도로 회피, 출발 시각별 소요, 도로별 구성 · 소통</sub></td>
<td width="50%"><img src="docs/images/road.png" alt="고속도로 실측"><br><sub><b>고속도로 실측 (길)</b> · 추석 귀성길 서울→대전 하행 11시 6시간 24분 — 영업소 구간 합 · 기준선 · 강수 예보 · 카카오 ETA</sub></td>
</tr>
<tr>
<td width="50%"><img src="docs/images/ops.png" alt="수집 상태"><br><sub><b>수집 상태</b> · 작업별 최근 실행 · 24h 완전성 · 공급자별 호출 예산 · 공개 지연 · 실행 이력</sub></td>
</tr>
</table>
...
| **전국 도로 분석** | 임의 두 지점: 도로별 구간 · 소통 · 고속도로 회피(국도 위주) 비교 · 출발 시각별 소요 (카카오 9건 병렬 · 20분 캐시) | `RoadClassTest` · E2E '도로 분석: 전국 어디든' |
...
| **실데이터 검증 · 코드 검토** | 실제 API 로 전체를 돌리며 발견한 문제와 코드 검토 결과 17건을 재현 → 수정 → 회귀 테스트로 고정 | [docs/VERIFICATION.md](docs/VERIFICATION.md) |
...
첫 기동 때 수집기가 자동으로 톨게이트 590개 동기화 → 도로·기상·대기·카카오 첫 수집 → **철도 92일 백필(276회 호출, 약 3분, 운행정보 83만 행)** → 기차역 276개 좌표 보정 → 기준선·백테스트를 실행합니다.
...
| `DATA_GO_KR_KEY` | 공공데이터포털 일반 인증키 (Decoding) | 코레일 열차운행정보 v2 · 기상청 단기예보 · 에어코리아 · 국토교통부(TAGO) 지하철정보 · **TAGO 열차정보**(역별 시간표 · 차종) (각각 활용신청) |
...
flowchart TB
  ...
  q -- "조회 시점 호출: 카카오 · 기상청 · 에어코리아 (같은 예산 · 캐시)" --> ext
  jobs -- "UPSERT (자연키)" --> pg
  ana --> pg
```
...
sequenceDiagram
  ...
  C->>DB: 받은 슬롯 범위만 길 합산 재계산 (H-v1 튀는 값 제거 → 채움 → 실측 60% 미만은 결측)
...
| `rail_geometry` | OpenStreetMap (Overpass) | 매일 05:00 확인 · 28일 지나면 갱신 · 첫 기동 | 4 (구역별 1회, 실패 시 미러를 바꿔 최대 6회 · 받은 구역은 3일 캐시 · 예산 60) |
| (API 요청 시) TAGO 열차 시간표 | 국토교통부 TAGO | 역 쌍 · 날짜를 처음 볼 때 한 번 | 역 쌍 · 날짜당 1 (DB 에 영구 저장 · 초당 10건 이하 · 예산 9,000) |
...
- **길 통행시간** = Σ 구간 통행시간 (1종 소형차, 도착기준 5분 슬롯). 구간 값이 없거나 제외되면 ±15분 최근접 → 24시간 중앙값 → 자유속도 순으로 채우고(`FILLED`), 실측 비율 60% 미만 슬롯은 결측.
- **품질 Q-v2 · H-v1** — [ADR-012](docs/adr/012-road-quality-rule.md).
- **기준선** — 길·방향·요일·슬롯별 최근 8주 p50·p90, 표본 n. n<4 이면 전체 요일 기준선으로 대체하고, 편차(%) 비교는 하지 않음(FR-402).
- **예측** — M0 = 기준선, M1 = M0(목표) + (현재 − M0(현재 슬롯)) × e^(−h/90분), 지속 = 현재. 도로공사 공개 지연 때문에 선행시간 h = 목표 − **마지막 관측 슬롯**.
- **정시성** — 계획 시각과 실제 시각을 정확히 비교한 값만 씁니다. 정시 = 도착 지연 ≤ 5분(설정).
  P-v1: 시발·종착역은 코레일 운행계획. P-t1: 중간역은 **TAGO 열차 시간표의 역별 계획 시각**(역 쌍 · 날짜별로 한 번 받아 `rail.tt_plan` 에 저장, [ADR-016](docs/adr/016-data-provenance-and-tago-timetable.md)).
...
| 2 | `GET /corridors` | 길 목록 (구간 체인 · 역 · 환경 지점 좌표) |
| 3 | `GET /corridors/{id}/now?dir&departIn&accessMin&carAccessMin` | 길 판단 카드 (고속도로 실측 기반) — road · rail · env · incidents · decision · freshness · caveat (`X-Cache`) |
...
| 층 | 대상 | 수 |
|---|---|---|
| 단위 (pytest) | 슬롯 정렬 · 격자 변환(기상청 격자표 4곳) · 품질 규칙 · 튀는 값 제거 · 길 합산 · 정시성(자정 넘김 · 조기 도착 · 확인 불가 · 보간) · 예측 골든 · 기준선 · 백테스트(결정성 · 누수 없음) · 돌발 매칭 · 역 좌표 매칭 · 선로 그래프(본선 추종 · 지선 · 다른 역 통과 금지 · 단순화 · 구역 캐시) · 공급자별 예산 설정 | 53 |
| 계약 (pytest) | 공급자 5종 실제 응답 fixture 파서 · 문자 안내 좌표(altitude = 경도, 범위 밖 · 좌표 없음은 버림) | 9 |
| 통합 (pytest + PostgreSQL · Redis) | 예산 동시성 · 멱등 수집 · 꼬리 커서 · 결측→백필 · seed 멱등 · 철도 일 계산 · SQL `od_trips` ↔ Python 계약 · **시간표(TT)가 보간보다 우선** · 호출 로그 키 마스킹 · 시간 초과가 오류 상세에 남는지 · 재시작 뒤 RUNNING·잠금·예약 정리 · 취소된 작업은 '중단'으로 기록 · 선로 작업 28일 안이면 건너뜀 | 17 |
| 단위 (JUnit) | 판단 규칙 · 예측 골든(Python 과 같은 파일) · 기상청 격자 · 행정구역→에어코리아 시도 · **CSA 환승 경로** · 도로 구분 · 경로 요약 · 돌발 좌표↔경로 거리 · TAGO 시각 해석·검증된 역명 별칭 | 42 |
| API 통합 (JUnit + Testcontainers) | 판단 카드 · 캐시 · 오류 규약 · 관리 API 401/202/409/400/429 · 수집 상태(오류 상세) · 헬스 · 임의 역 쌍(OO발 OO행 · 추정 차종 없음 · 가나다순) · 출발지→도착지(선로 경로) | 8 |
| E2E (Playwright) | 판단 · ⇄ 교환 · 전국 검색 · 검색 칸 빈 목록 · 서비스 소개 · 판단 근거 돌발 목록 · 카드(도착 예정·시간 구성·OSM 출처) · 도로 분석 · 지도 잠금 · 선택 목록 잘림 · 고속도로 실측 · 철도 역 검색 · 가나다순 · 차종은 TAGO 배지로만 · 수집 상태 오류 복사 · 모바일 메뉴 | 15 |
...
| 서울역→대전역 최근 30일 | 정시율 89.9% · 평균 도착 지연 2.5분 · p90 5.1분 · 3,586회 (선형 보간 P-i1 기준, 첫날) |
| 같은 구간 · TAGO 시간표 비교 (09-25) | 08-26~09-24 정시율 **83.9%** · 평균 지연 3.0분 · p90 7.0분 · 확인 3,257회 / 확인 불가 347회(TAGO 시간표가 없는 날) — 보간이 지연을 과소 추정했음 |
...
- 기차 환승은 코레일 여객열차끼리만 다룹니다. 지하철·버스를 포함한 대중교통 환승 경로는 공개 데이터(TAGO 는 역별 시각만 제공)로 만들 수 없어 제외했습니다 — ODsay 같은 대중교통 길찾기 API 를 붙이면 확장할 수 있습니다.
- 역까지 도보(1km 미만)는 공개 도보 길찾기 API 가 없어 직선 × 1.3 ÷ 4.5km/h 로 추정하고 '도보 추정'으로 표시합니다(TMAP 보행자 API 키가 있으면 실제 값으로 바꿀 수 있음).
- 카카오 경로에는 도로 등급이 없어 국도·지방도·시내 도로를 나누지 않고 '그 외 도로'로 표시합니다. 통행료도 카카오가 0원을 돌려줘(서울→대전에도 0원) 표시하지 않습니다.
...
- 차종은 TAGO 시간표가 있는 날짜 · 역 쌍에만 표시됩니다. 수서발 열차는 TAGO 가 차량 종류(예: KTX-산천)로 적어 줍니다.
