 ...
 *  (1) 추천 경로의 도로별 구성(고속도로 · 도시고속 · 국도 · 일반도로)과 느린 구간,
 *  (2) 고속도로 회피(국도 · 일반도로 위주) 경로 비교,
...
public class RoadRouteService {
    ...
    public Analysis analyze(TripDtos.Place from, TripDtos.Place to, int departIn) {
        ...
        return new Analysis(from, to, Math.round(KmaGrid.km(from.lat(), from.lon(), to.lat(), to.lon()) * 10) / 10.0, depart,
                summarize("추천 경로", rec), summarize("고속도로 회피 (국도·일반도로 위주)", avoid), profile, best,
                obs == null ? null : new Monitored(obs.corridorId(), obs.corridorName(), obs.direction()),
                "카카오 미래 운행 정보(출발 시각 기준 예측). 도로 종류는 도로 이름으로 분류했습니다(국도 구간이 도로명으로 표기되면 일반도로로 집계).");
    }

    static RouteSummary summarize(String label, KakaoMobilityClient.Route r) {
        if (r == null) return new RouteSummary(label, null, null, null, null, List.of(), List.of(), List.of(), List.of());
        ...
        for (String t : List.of(RoadClass.MOTORWAY, RoadClass.URBAN_EXPRESS, RoadClass.NATIONAL, RoadClass.LOCAL)) agg.put(t, new int[2]);
        for (var run : runs) {
        ...
        List<RoadRun> major = runs.stream().filter(x -> x.distanceM() >= 1000 || !"일반도로".equals(x.type())).toList();
        return new RouteSummary(label, r.durationSec(), r.distanceM(), r.tollFare(), r.taxiFare(), r.path(), shares, major, slow);
    }
